...
def appendUniqueWordsAsColumnsForHam(tokens):
	for token in tokens:
		if token not in stopWords.wordsToBeIgnored:
...
stringForHam = "\n\nAccuracy with test dataset for Ham with stop words excluded:" + str(getAccuracyForHamDocs())

stringForSpam = "\nAccuracy with test dataset for Spam with stop words excluded:" + str(getAccuracyForSpamDocs())
